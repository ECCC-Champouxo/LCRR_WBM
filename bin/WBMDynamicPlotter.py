...
class WBM_DYNAMIC_PLOTTER(object):
    ...
    _parser=None
    _output_folder=os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', 'output'))
    ...
    def __init__(self,parser=None,sector=None,flow=None):
        self._parser=parser
        self.sector=sector
        if self.sector=='LAKE':
            ...
            self.x='LEVEL_MARINA_x'
            self.y ='LEVEL_MARINA_y'
            self.tag_x ='@LEVEL_MARINA_x'
            self.tag_y ='@LEVEL_MARINA_y'
        ...
        output_file(os.path.join(self._graphs_folder,''.join(('Results_{}_{}_{}.html'.format(NBS_supply_scenario,Shoal_modification,self.name)))))
     ...
     #background_fill_color="#efefef", x_range=(dates_Out_df[22280], dates_Out_df[22644]))
        p = figure(plot_height=int(CurrentVerticalResolution*0.55), plot_width=int(CurrentHorizontalResolution*0.95), tools=[hover,"save","xpan","wheel_zoom","reset","box_zoom"], toolbar_location="above",
                x_axis_type="datetime", x_axis_location="above",
                background_fill_color="#efefef", x_range=(dates_Out_df[0], dates_Out_df[-1]))

        red_renderer_line1 =p.line('DATE', self.x, source=source,color='#A6CEE3',line_width=1, legend=self._parser.get('GRAPHICS','Calculated_legend_label'))
        red_renderer_line2 =p.line('DATE', self.y, source=source, color='#FB9A99',line_width=1, legend=self._parser.get('GRAPHICS','Baseline_legend_label'))
        p.add_tools(HoverTool(tooltips=[("DATE", '@DateString'),(self._parser.get('GRAPHICS','Baseline'), self.tag_y),(self._parser.get('GRAPHICS','Calculated_legend_label'), self.tag_x),("Differences", "@Diff")],
        formatters={'date': 'datetime'},mode="vline",renderers=[red_renderer_line1,red_renderer_line2]))


        p.yaxis.axis_label = '{} water level (m-NAVD88)'.format(self.name)
